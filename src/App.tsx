...
function SpaceInvadersApp() {
    ...
    if (saved) {
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          console.warn('Saved session is not an array, resetting');
          return [];
        }
      } catch (e) {
  ...
  const addWord = useCallback((text: string) => {
    ...
    setTextBuffer(prev => {
      // Ensure prev is an array (safety check)
      const safePrev = Array.isArray(prev) ? prev : [];
      const newBuffer = [...safePrev, text];
      
      // Save to localStorage
      localStorage.setItem('voidwriter-session', JSON.stringify(newBuffer));
  ...
  const completeWord = useCallback(() => {
    if (currentWord.trim()) {
      addWord(currentWord);
      setCurrentWord('');
      // Clear saved current word
      localStorage.removeItem('voidwriter-current');
  ...
  const createTypingParticle = useCallback(() => {
    ...
    setTypingParticles(prev => {
      const safePrev = Array.isArray(prev) ? prev : [];
      const filtered = safePrev.filter(p => p.life > 0);
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    ...
    if (/^[a-zA-Z]$/.test(event.key)) {
      setCurrentWord(prev => {
        ...
        localStorage.setItem('voidwriter-current', updated);
        return updated;
      });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setTypingParticles(prev => {
        const safePrev = Array.isArray(prev) ? prev : [];
        return safePrev.filter(p => p.life > 0);
      });
    }, 2000);
    return () => clearInterval(interval);
  }, []);
  ...
  const clearAll = useCallback(() => {
    ...
    setTextBuffer([]);
    // Clear localStorage
    localStorage.removeItem('voidwriter-session');
    localStorage.removeItem('voidwriter-current');
  ...
  const showFeedback = useCallback((message: string) => {
    ...
    setActionFeedback(prev => [...prev, feedback]);
  ...
  const handleDownload = useCallback(() => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `skywriter-${Date.now()}.txt`;
    link.click();
    URL.revokeObjectURL(url);
    showFeedback('SAVED');
    ...
    <div className="app" style={{ background: '#000' }}>
      ...
      >
        ...
        <div style={{ maxWidth: '250px' }}>
          <div style={{ fontSize: '8px', opacity: 0.7, marginBottom: '8px' }}>BUFFER</div>
          <div style={{ 
            fontSize: '8px', 
            opacity: 0.9,
            lineHeight: '1.6'
          }}>
            {textBuffer.slice(-3).join(' ')}
          </div>
        </div>
      </div>
